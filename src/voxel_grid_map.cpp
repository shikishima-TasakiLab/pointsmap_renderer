...
template <class VoxelGridMapPtrT>
VoxelGridMap<VoxelGridMapPtrT>::VoxelGridMap(const VoxelGridMapPtrT &vgm) {this->_voxel_grid_map = vgm;}

template <class VoxelGridMapPtrT>
bool VoxelGridMap<VoxelGridMapPtrT>::is_empty() {return this->_voxel_grid_map->Z.size() == 0UL;}
...
void VoxelGridMap<VoxelGridMapPtrT>::create_depthmap(
    ...
    CameraParam camera_param = {
        .Fx = static_cast<float_t>(camerainfo->K[K::Fx]),
        .Fy = static_cast<float_t>(camerainfo->K[K::Fy]),
        .Cx = static_cast<float_t>(camerainfo->K[K::Cx]),
        .Cy = static_cast<float_t>(camerainfo->K[K::Cy]),
        .height = static_cast<size_t>(camerainfo->height),
        .width = static_cast<size_t>(camerainfo->width)
    };
    XY<float_t> shape_f = {.x = static_cast<float_t>(camera_param.width), .y = static_cast<float_t>(camerainfo->height)};
    LRTB<float_t> tan = {
        .l = -camera_param.Cx / camera_param.Fx,
        .r = (shape_f.x - camera_param.Cx) / camera_param.Fx,
        .t = -camera_param.Cy / camera_param.Fy,
        .b = (shape_f.y - camera_param.Cy) / camera_param.Fy
    };
...
template <class VoxelGridMapPtrT>
void VoxelGridMap<VoxelGridMapPtrT>::_invert_transform(const Eigen::Vector3f &in_tr, const Eigen::Quaternionf &in_q, Eigen::Vector3f &out_tr, Eigen::Quaternionf &out_q)
{
...
template <class VoxelGridMapPtrT>
void VoxelGridMap<VoxelGridMapPtrT>::_depth_filter(const pcl::PointCloud<pcl::PointXYZ> &src, pcl::PointCloud<pcl::PointXYZ> &dst, const Range &depth_range)
{
    size_t len = src.points.size();

    if (depth_range.min < 0.0F || depth_range.min == NAN) throw std::runtime_error("\"depth_range.min\" must be longer than 0.");
    if (depth_range.max <= 0.0F || depth_range.max == NAN) throw std::runtime_error("\"depth_range.max\" must be longer than 0.");
    if (len == 0UL) {
        dst = src;
        return;
    }
...
template <>
void VoxelGridMap<pointsmap_renderer::VoxelGridMapPtr>::_set_voxel_grid(const pcl::PointCloud<pcl::PointXYZ> &pointsmap, const std::string &frame_id, const float_t voxel_size)
{
    this->_voxel_grid_map.reset(new pointsmap_renderer::VoxelGridMap());
    ...
    XYZ<long> voxels_minus = {
        .x = static_cast<long>(std::roundf((min_pt.x - this->_voxel_grid_map->voxels_center.x) / this->_voxel_grid_map->voxel_size - 0.5F)),
        .y = static_cast<long>(std::roundf((min_pt.y - this->_voxel_grid_map->voxels_center.y) / this->_voxel_grid_map->voxel_size - 0.5F)),
        .z = static_cast<long>(std::roundf((min_pt.z - this->_voxel_grid_map->voxels_center.z) / this->_voxel_grid_map->voxel_size - 0.5F)),
    };
    XYZ<long> voxels_plus = {
        .x = static_cast<long>(std::roundf((max_pt.x - this->_voxel_grid_map->voxels_center.x) / this->_voxel_grid_map->voxel_size + 0.5F)),
        .y = static_cast<long>(std::roundf((max_pt.y - this->_voxel_grid_map->voxels_center.y) / this->_voxel_grid_map->voxel_size + 0.5F)),
        .z = static_cast<long>(std::roundf((max_pt.z - this->_voxel_grid_map->voxels_center.z) / this->_voxel_grid_map->voxel_size + 0.5F))
    };
    ...
    for (size_t z = 0UL; z < this->_voxel_grid_map->voxels_len.z; z++) {
        pointsmap_renderer::Voxel_XY tmp_voxel_xy;

        for (size_t y = 0UL; y < this->_voxel_grid_map->voxels_len.y; y++) {
            pointsmap_renderer::Voxel_X tmp_voxel_x;

            for (size_t x = 0UL; x < this->_voxel_grid_map->voxels_len.x; x++) {
                pointsmap_renderer::VoxelPoints tmp_voxelpoints;
    ...
    ROS_INFO("Load %lu points.", pointsmap.size());
}

template <class VoxelGridMapPtrT>
void VoxelGridMap<VoxelGridMapPtrT>::_create_frustum_points(const Range &depth_range, const LRTB<float_t> &tan, pcl::PointCloud<pcl::PointXYZ> &dst)
{
    dst.points.resize(9);
    ...
    //  前面左下
    pcl::PointXYZ front_bottom_left;
    ...
    //  前面右下
    pcl::PointXYZ front_bottom_right;
    ...
    //  後面右下
    pcl::PointXYZ back_bottom_right;
...
template <class VoxelGridMapPtrT>
bool VoxelGridMap<VoxelGridMapPtrT>::_voxel_frontside(const pointsmap_renderer::VoxelPoints &voxel, const Eigen::Vector3f &normal, const Eigen::Vector3f &point_on_plane)
{
    //  平面から最も遠い点を導出
...
void VoxelGridMap<VoxelGridMapPtrT>::_voxels_include_frustum(
    ...
    pcl::PointCloud<pcl::PointXYZ> frustum_points;
...
template <class VoxelGridMapPtrT>
void VoxelGridMap<VoxelGridMapPtrT>::_voxels_include_frustum(const pcl::PointCloud<pcl::PointXYZ> &frustum_points, const Range &depth_range, std::vector<XYZ<size_t> > &dst_voxel_indexs)
{
    /*  0: FrontBottomLeft
        1: FrontTopLeft
        2: FrontTopRight
    ...
    #endif
    {
        ...
        for (size_t i = 0ul; i < voxels_len; i++) {
            ...
            for (size_t p = 0; p < p_len; p++) {
                pcl::PointXYZ *src_point = &(points.points[p]);
...
template <>
void VoxelGridMap<pointsmap_renderer::VoxelGridMapPtr>::set_pointsmap(const std::vector<std::string> &paths, const std::string &frame_id, const float_t voxel_size)
{
    size_t paths_len = paths.size();
    pcl::PointCloud<pcl::PointXYZ> pointsmap, part;

    for (size_t i = 0UL; i < paths_len; i++) {
        if (pcl::io::loadPCDFile(paths[i], part) == -1) {
            ROS_ERROR("Load failed \"%s\"", paths[i].c_str());
        }
        else {
            ROS_INFO("Load \"%s\" (%lu points)", paths[i].c_str(), part.size());
            pointsmap += part;
        }
    }
...
template <>
void VoxelGridMap<pointsmap_renderer::VoxelGridMapPtr>::set_pointsmap(const std::string &path, const std::string &frame_id, const float_t voxel_size)
{
...
template <>
void VoxelGridMap<pointsmap_renderer::VoxelGridMapPtr>::set_pointsmap(const sensor_msgs::PointCloud2ConstPtr pointsmap, const float_t voxel_size)
{
    pcl::PointCloud<pcl::PointXYZ> tmp_points;
    pcl::fromROSMsg(*pointsmap, tmp_points);
    this->_set_voxel_grid(tmp_points, pointsmap->header.frame_id, voxel_size);
}

template <>
VoxelGridMap<pointsmap_renderer::VoxelGridMapPtr>::VoxelGridMap(const std::string &path, const std::string &frame_id, const float_t voxel_size) {this->set_pointsmap(path, frame_id, voxel_size);}

template <>
VoxelGridMap<pointsmap_renderer::VoxelGridMapPtr>::VoxelGridMap(const std::vector<std::string> &paths, const std::string &frame_id, const float_t voxel_size) {this->set_pointsmap(paths, frame_id, voxel_size);}

template <>
VoxelGridMap<pointsmap_renderer::VoxelGridMapPtr>::VoxelGridMap(const sensor_msgs::PointCloud2ConstPtr pointsmap, const float_t voxel_size) {this->set_pointsmap(pointsmap, voxel_size);}


template class VoxelGridMap<pointsmap_renderer::VoxelGridMapPtr>;
template class VoxelGridMap<pointsmap_renderer::VoxelGridMapConstPtr>;

template void VoxelGridMap<pointsmap_renderer::VoxelGridMapPtr>::create_depthmap(
    const sensor_msgs::CameraInfoPtr &camerainfo,
    const geometry_msgs::TransformStamped &tf,
    cv::Mat &depthmap,
    const Range &depth_range
);
template void VoxelGridMap<pointsmap_renderer::VoxelGridMapConstPtr>::create_depthmap(
    const sensor_msgs::CameraInfoPtr &camerainfo,
    const geometry_msgs::TransformStamped &tf,
    cv::Mat &depthmap,
    const Range &depth_range
